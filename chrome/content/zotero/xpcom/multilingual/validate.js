...
Zotero.ZlsValidator = function () {
	...
	this.getPrimary = Zotero.Promise.coroutine(function* () {
		...
		if (!primary_subtag) {
			...
			if (this.remnant.length && this.remnant[0]) {
				...
				var res = yield Zotero.DB.valueQueryAsync(sql, [this.remnant[0]]);
				if (res) {
					this.remnant[0] = res;
				}
				var testlen = this.remnant[0].length;
			}
			yield this.testPrimary(2);
			while (yield this.testPrimary(3)) {
	 ...
	 */
	this.testPrimary = Zotero.Promise.coroutine(function* (len) {
		var primary_subtag;
		...
		if (primary_subtag.length === len) {
			yield this.checkPrimarySql(primary_subtag);
			if (this.tagdata) {
	 ...
	 * to false.
	 */
	this.checkPrimarySql = Zotero.Promise.coroutine(function* (primary) {
	...
	this.getScript = Zotero.Promise.coroutine(function* () {
		if (!this.remnant.length) {
			return;
		}
		...
		var res = yield Zotero.DB.rowQueryAsync(sql,['script',this.remnant[0]]);
		if (res) {
			res.type = 'script';
	...
	this.getRegion = Zotero.Promise.coroutine(function* () {
		if (!this.remnant.length) {
			return;
		}
		...
		var sql = 'SELECT TA.value AS subtag, D.value AS description FROM zlsSubtags S '
			...
			+ 'LEFT JOIN zlsSubTagData D ON S.description=D.id '
			+ 'WHERE TY.value=? AND TA.value=?';
		var res = yield Zotero.DB.rowQueryAsync(sql,['region',this.remnant[0]]);
		if (res) {
			res.type = 'region';
	...
	this.getVariant = Zotero.Promise.coroutine(function* () {
		if (!this.remnant.length) {
			return;
		}
		...
		for (var i = 0, ilen = this.remnant.length; i < ilen; i += 1) {
			yield this._getVariant();
		}
	});
	
	this._getVariant = Zotero.Promise.coroutine(function* () {
		var myprefix = [];
		...
		var res = yield Zotero.DB.rowQueryAsync(sql,['variant',this.remnant[0]]);		
		if (res) {
			res.type = 'variant';
