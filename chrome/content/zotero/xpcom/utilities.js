...
Zotero.Utilities = {
	...
	"itemToCSLJSON":function(zoteroItem, ignoreURL, portableJSON, stopAuthority) {
		if (zoteroItem instanceof Zotero.Item) {
			return Zotero.Utilities.Internal.itemToExportFormat(zoteroItem).
				then(function(zoteroItem) {
                    Zotero.Utilities.itemToCSLJSON(zoteroItem, ignoreURL, portableJSON, stopAuthority);
                });
		}
	...
	"getCourtName": Zotero.Promise.coroutine(function* (jurisdictionID, courtID, fallback) {
		...
		var res = yield Zotero.DB.valueQueryAsync(sql,[courtID,jurisdictionID,countryID]);
		return res || !fallback ? res : courtID;
	}),

	"getCourtID": Zotero.Promise.coroutine(function* (jurisdictionID, courtName, fallback) {
		var countryID = jurisdictionID.split(":")[0];
		...
		var res = yield Zotero.DB.valueQueryAsync(sql,[courtName,jurisdictionID,countryID]);
		return res || !fallback ? res : courtName;
	}),

	"getJurisdictionName": Zotero.Promise.coroutine(function* (jurisdictionID, fallback) {
		var sql = "SELECT jurisdictionName FROM jurisdictions "
			+ "WHERE jurisdictionID=?;";
		var res = yield Zotero.DB.valueQueryAsync(sql, [jurisdictionID]);
		return res || !fallback ? res : jurisdictionID;
	}),

	"getJurisdictionID": Zotero.Promise.coroutine(function* (jurisdictionName, fallback) {
		var sql = "SELECT jurisdictionID FROM jurisdictions "
			+ "WHERE jurisdictionName=? OR jurisdictionName LIKE ?;";
		var res = yield Zotero.DB.valueQueryAsync(sql, [jurisdictionName, '%|' + jurisdictionName]);
