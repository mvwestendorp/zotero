...
Zotero_Preferences.Lang = {
	...
	init: Zotero.Promise.coroutine(function* () {
        ...
        var startTime = Date.now();
 	    ...
 	    yield Zotero_Preferences.Lang.refreshMenus();
    ...
    refreshMenus: Zotero.Promise.coroutine(function* () {
 	    yield Zotero_Preferences.Lang.refreshScriptMenu();
 	    yield Zotero_Preferences.Lang.refreshRegionMenu();
    }),
    ...
    scriptLangMenuPrep: Zotero.Promise.coroutine(function* (topnode) {
        // Promise
 	    ...
 	    var script = yield Zotero.DB.columnQueryAsync(sql,['language',tag]);
    ...
    selectScript: function (node) {
        ...
        Zotero_Preferences.Lang.insertLanguageRow(tag);
    },
    ...
    refreshLanguages: function() {
        // Promise
 	    ...
 	    var tags = Zotero.CachedLanguages.getLangTags();
 	    for (var i = 0, ilen = tags.length; i < ilen; i += 1) {
 			var row = Zotero_Preferences.Lang.addLangRow(parent, tags[i].nickname, tags[i].tagdata);
 			row.setAttribute("class", "compact");
 			Zotero_Preferences.Lang.addSelectors(row, tags[i].tag);
         ...
         */
 	    if (textbox.value) {
 		    ...
 		    if (!comment) {
 				...
 				Zotero_Preferences.Lang.insertLanguageRow(comment);
 				textbox.value = '';
 				textbox.blur();
 		    }
 	    }
    ...
    insertLanguageRow: Zotero.Promise.coroutine(function* (tag) {
        yield Zotero.CachedLanguages.addTag(tag);
 	    Zotero_Preferences.Lang.refreshLanguages();
    }),
    ...
    setRemoveDisable: Zotero.Promise.coroutine(function* (button, tag) {
        /*
         * Disable or enable the delete button on language rows,
         * as appropriate.
         */
 	    if (Zotero.CachedLanguages.hasTagDependents(tag)) {
    ...
    deleteTag: Zotero.Promise.coroutine(function* (row) {
        // Promise -- create function in CachedLanguages and yield from there
        /*
        ...
        yield Zotero.CachedLanguages.deleteTag(tag);
 	    Zotero_Preferences.Lang.refreshLanguages();
    }),
    ...
    hideNicknameEditor: Zotero.Promise.coroutine(function* (textbox) {
        var nickname = textbox.value;
        var nicknameExists = Zotero.CachedLanguages.nicknameExists(nickname);
 	    if (textbox.value !== textbox.getAttribute('value') && nicknameExists) {
 		    return;
 	    }
 	    var oldNickname = textbox.getAttribute('value');
 	    var newNickname = textbox.value;
        newNickname = yield Zotero.CachedLanguages.nicknameReset(oldNickname, newNickname);
 	    var parent = textbox.parentNode;
 	    parent.setAttribute('onclick', 'Zotero_Preferences.Lang.showNicknameEditor(this.firstChild)');
 	    var label = document.createElement('label');
 	    label.setAttribute("value",newNickname);
    ...
    buildSelector: function (profile,tag,param) {
 	    var checkbox = document.createElement('checkbox');
 	    if (Zotero.CachedLanguages.langPrefIsSet(profile,param,tag)) {
    ...
    setLangPref: Zotero.Promise.coroutine(function* (event) {
        ...
        var langTag = target.getAttribute('value');
        var checked = target.hasAttribute('checked');
        yield Zotero.CachedLanguages.changePref(profile, param, langTag, checked);
 	    var langRow = document.getElementById(langTag+'::row');
 	    var removeButton = langRow.lastChild.lastChild.previousSibling;
 	    yield Zotero_Preferences.Lang.setRemoveDisable(removeButton,langTag);
    }),
