...
Zotero_Preferences.Lang = {
    ...
    refreshScriptMenu: Zotero.Promise.coroutine(function* () {
 	    ...
 	    if (!box.childNodes.length) {
 		    var sql = 'SELECT TA.value AS subtag, D.value AS description FROM zlsSubtags S '
 			    ...
 			    + 'WHERE TY.value=? '
 			    + 'ORDER BY D.value';
 		    var res = yield Zotero.DB.queryAsync(sql,['script']);
    ...
    selectScript: Zotero.Promise.coroutine(function* (node) {
        Zotero.debug("zls: selectScript()");
 	    ...
 	    yield Zotero_Preferences.Lang.handleDependentLanguageRowInsert(tag);
    }),
    
    selectRegion: Zotero.Promise.coroutine(function* (node) {
 	    var parent = node.parentNode;
 	    var topnode = document.getElementById('extend-lang-menu');
 	    var rowId = topnode.getAttribute('target-row-id');
 	    var tag = rowId.slice(0,-5);
    ...
    selectVariant: Zotero.Promise.coroutine(function* (node) {
 	    var parent = node.parentNode;
 	    var topnode = document.getElementById('extend-lang-menu');
 	    var rowId = topnode.getAttribute('target-row-id');
 	    var tag = rowId.slice(0,-5);
    ...
    handleDependentLanguageRowInsert: Zotero.Promise.coroutine(function* (tag) {
        Zotero.debug("zls: tag for validation: ("+tag+")");
 	    var tagdata = yield Zotero.subtagRegistry.validate(tag);
 	    if (tagdata) {
 		    Zotero_Preferences.Lang.insertLanguageRow(tagdata);
 	    }
    }),
 	
    refreshRegionMenu: Zotero.Promise.coroutine(function* () {
 	    ...
 	    if (!box.childNodes.length) {
 		    var sql = 'SELECT TA.value AS subtag, D.value AS description FROM zlsSubtags S '
 			    ...
 			    + 'WHERE TY.value=? '
 			    + 'ORDER BY D.value';
 		    var res = yield Zotero.DB.queryAsync(sql,['region']);
 		    for (var i = 0, ilen = res.length; i < ilen; i += 1) {
 			    var item = document.createElement('menuitem');
    ...
    scriptLangMenuPrep: Zotero.Promise.coroutine(function* (topnode) {
 	    ...
 	    var script = yield Zotero.DB.columnQuery(sql,['language',tag]);
    ...
    variantLangMenuPrep: Zotero.Promise.coroutine(function* (topnode) {
 	    ...
 	    var res = yield Zotero.DB.queryAsync(sql,['variant',searchtag,existing_variants]);
 	    for (var i = 0, ilen = res.length; i < ilen; i += 1) {
 		    var item = document.createElement('menuitem');
 		    item.setAttribute('label',res[i].description);
    ...
    refreshLanguages: Zotero.Promise.coroutine(function* () {
 	    ...
 	    for (var i = 0, ilen = tags.length; i < ilen; i += 1) {
 		    var tagdata = yield Zotero.subtagRegistry.validate(tags[i].tag);
 		    if (tagdata) {
 			    var row = yield Zotero_Preferences.Lang.addLangRow(parent, tags[i].nickname, tagdata);
    ...
    addLangRow: Zotero.Promise.coroutine(function* (parent, nickname, tagdata) {
        ...
        //newrow.setAttribute("maxwidth","600");	
 	    ...
 	    // Set tags
 	    if (tagdata.length) {
 		    yield Zotero_Preferences.Lang.addSubtag(secondhbox, tagdata[0]);		
 	    }
    ...
    addSubtag: Zotero.Promise.coroutine(function* (box, subtagdata) {
 	    var label = document.createElement('label');
    ...
    handleLangAutoCompleteSelect: Zotero.Promise.coroutine(function* (textbox) {
        /*
         ...
         */
 	    if (textbox.value) {
 		    ...
 		    if (!comment) {
				...
				var tagdata = yield Zotero.subtagRegistry.validate(comment);
 			    if (tagdata) {
 				    Zotero_Preferences.Lang.insertLanguageRow(tagdata);
 				    textbox.value = '';
 				    textbox.blur();
    ...
    insertLanguageRow: Zotero.Promise.coroutine(function* (tagdata) {
 	    ...
 	    Zotero.CachedLanguages.init();
    }),
    
    extendLanguageTopMenu: Zotero.Promise.coroutine(function* (row) {
    ...
    setRemoveDisable: function (button, tag) {
         ...
         * as appropriate.
         */
 	    if (Zotero.CachedLanguages.usesTag(tag)) {
    ...
    deleteTag: Zotero.Promise.coroutine(function* (row) {
        /*
    ...
    tagDependents: Zotero.Promise.coroutine(function* (tag) {
        /*
         ...
         */
 	    ...
 	    if (!hasDependents) {
 		    ...
 		    hasDependents = yield Zotero.DB.valueQueryAsync(sql, [tag]);
 	    }
 	    return hasDependents;
    }),
    
    nicknameExists: Zotero.Promise.coroutine(function* (nickname) {
        /*
         ...
         */
 	    ...
 	    var result = yield Zotero.DB.valueQueryAsync(sql,[nickname]);
 	    return result;
    }),
    ...
    hideNicknameEditor: function (textbox) {
 	    if (!textbox.value || Zotero.CachedLanguages.nicknameExists(textbox.value)) {
 	    ...
 	    Zotero.DB.queryAsync(sql,[newval,oldval])
		.then(Zotero.CachedLanguages.init());
 	    //updateSelectors(parent.parentNode, parent.parentNode.id.slice(-5));
    },
    
    addSelectors: Zotero.Promise.coroutine(function* (row, tag) {
 	    ...
 	    for (var j = 0, jlen = languageSelectorTypes.length; j < jlen; j += 1) {
 		    var newselector = yield Zotero_Preferences.Lang.buildSelector('default',tag,languageSelectorTypes[j]);
 		    if ((j % 5) == 2) {
    ...
    buildSelector: Zotero.Promise.coroutine(function* (profile,tagdata,param) {
 	    var checkbox = document.createElement('checkbox');
 	    if (yield Zotero_Preferences.Lang.langPrefIsSet(profile,tagdata.tag,param)) {
    ...
    setLangPref: Zotero.Promise.coroutine(function* (event) {
 	    var target = event.target;
